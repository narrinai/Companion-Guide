...
/**
 * Remove duplicate English translation records
 * Keep only one EN record per companion (the most complete one)
 */
 ...
 */
async function main() {
  console.log('\n🧹 Removing Duplicate English Translation Records\n');
  ...
  try {
    ...
    for (const [companionId, records] of Object.entries(groupedByCompanion)) {
      if (records.length > 1) {
        duplicatesFound++;
        console.log(`\n🔍 Found ${records.length} English records for companion: ${companionId}`);
    ...
    console.log(`   Remaining: ${englishRecords.length - deletedCount} English records`);
    console.log('\n');
